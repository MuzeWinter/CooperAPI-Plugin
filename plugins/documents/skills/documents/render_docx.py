...
def _prepend_windows_libreoffice_bin() -> None:
    """Find a standard Windows LibreOffice install before PATH is refreshed."""
    ...
    for candidate_dir in candidate_dirs:
        if not os.path.isdir(candidate_dir):
            continue
        if candidate_dir in path_entries:
            return

        os.environ["PATH"] = os.pathsep.join(
            [candidate_dir, *[entry for entry in path_entries if entry]]
        )
        return
...
def convert_to_pdf(
    ...
    _prepend_windows_libreoffice_bin()
    _default_macos_tmpdir_for_soffice()
    env = _build_lo_env(user_profile)
    user_installation_uri = Path(user_profile).resolve().as_uri()
    logs: list[str] = []
    ...
    cmd_pdf = [
        "soffice",
        "-env:UserInstallation=" + user_installation_uri,
        "--invisible",
        "--headless",
        "--norestore",
        "--convert-to",
    ...
    with tempfile.TemporaryDirectory(prefix="soffice_profile_") as user_profile:
        with tempfile.TemporaryDirectory(prefix="soffice_convert_") as convert_tmp_dir:
            ...
            try:
                ...
                # Windows installs often have PyMuPDF available but no Poppler
                # binaries. Use it as a page-size fallback when pdfinfo is absent.
                import fitz

                with fitz.open(pdf_path) as pdf_doc:
                    page_rect = pdf_doc.load_page(0).rect
    ...
    with tempfile.TemporaryDirectory(prefix="soffice_profile_") as user_profile:
        with tempfile.TemporaryDirectory(prefix="soffice_convert_") as convert_tmp_dir:
            ...
            try:
                ...
                if verbose:
                    print("[render_docx] pdf2image failed; falling back to PyMuPDF: " + str(exc))
