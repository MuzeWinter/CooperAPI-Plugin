...
- If no Databricks plugin, app, or connector setup route is exposed in this Codex/Data Analytics session, keep the category as `needs_confirmation`, `missing`, `unavailable`, `deferred`, or `skipped`, say Databricks is not exposed as a first-class app or connector in this Codex/Data Analytics session, ask the user to ask their ChatGPT or workspace admin to enable Databricks for the workspace, then offer skip, or manual or exported fallback.
