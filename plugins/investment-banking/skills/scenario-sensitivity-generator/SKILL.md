...
Apply the presentation-surface precedence in `../../references/deliverable-intake-policy.md`. This workflow's natural artifact is an XLSX sensitivity workbook. Do not choose chat-only output unless the user explicitly requests a lightweight response.
...
- `../../references/workbook-first-tab-standard.md`: required for substantive workbook sensitivity packs.
- `../../references/html-artifact-standard.md`: read only when a standalone HTML sensitivity companion is explicitly requested or selected.
