...
```json
{
  "connector_confirmation": {
    "structured_data": {
      ...
      "connector": {
        "id": "asdk_app_<resolved_databricks_app_id>",
        "name": "Databricks"
      },
...
2. Check the session `Available plugins` block for an installed plugin whose name, display name, id, or declared app or connector ids match a configured preferred plugin route for the category. If it has visible plugin-owned skill or tool surface and, when the plugin depends on an app or connector, the relevant app or connector surface is exposed or callable, write it as the active route. If the plugin is installed but its required app or connector surface is not exposed, do not mark it active; continue to the app or connector setup path below.
3. If no installed related plugin exists, or an installed related plugin still needs its required app or connector setup route, call `functions.list_available_plugins_to_install` if it has not already been called in this setup pass, then look for related plugin candidates.
...
5. Prefer plugin candidates over connector candidates even when the app or connector route is already callable or active. Treat one related plugin candidate as clearly best only when the user already saved that source preference or explicitly chose it in the current turn. Configured `preferred_plugins` order, legacy `preferred_apps` order when present, and connector-id intersection rank related candidates, but do not by themselves select one mutually exclusive provider for the user. If one candidate is clearly best by saved or current-turn user choice, ask one clear install or defer question unless the user explicitly requested installation. If the user confirms, call `functions.request_plugin_install` with the exact returned candidate `tool_type` and `id`. Do not call `request_plugin_install` in parallel with any other tool.
6. If a mutually exclusive category has multiple configured `preferred_plugins` and the user has not saved or made a source choice, ask the user to choose among the relevant configured providers instead of collapsing to the first-ranked or only currently installable candidate.
...
9. If no related plugin candidate exists, or an installed related plugin still needs its required app or connector setup route, use the same install-list results and exposed session tools to find an installed or installable app or connector candidate.
...
```json
{
  ...
  "name": "Slack",
  "display_name": "Slack",
  "app_connector_ids": ["asdk_app_69a1d78e929881919bba0dbda1f6436d"],
  "description": "Use Slack messages and Data Analytics workflows from Codex."
}
```
...
- `missing or unsupported`: the connector or app is not installed, not exposed, or lacks the needed action. When a configured warehouse source such as Databricks, BigQuery, or Snowflake is not exposed in this Codex/Data Analytics session, say that clearly, ask the user to ask their ChatGPT or workspace admin to enable the app or connector for the workspace, then offer skip, or manual or exported fallback.
...
Keep user-facing wording practical. Say what source is unavailable and why it matters for the analytical job, such as `I don’t see a Databricks app or connector exposed in this Codex/Data Analytics session, so I cannot confirm metric definitions from live tables yet. Ask your ChatGPT or workspace admin to enable Databricks for this workspace, or provide Databricks CLI output, SQL/schema context, or exported query results for now.`
...
- `missing`: setup found no matching installed plugin, connector, or app. Explain what Data Analytics workflows will be weaker without it and list concrete IT or admin options, such as installing the plugin or connector, authorizing the app for Codex, or using a manual or exported fallback for now.
