...
4. Select the delivery path that matches the final surface.
   - Default to Data Analytics MCP chart widgets for ad hoc, inline, chat-visible, or otherwise unspecified chart requests. Treat "make/show/render this chart" as an MCP widget request unless the user explicitly asks for Python, a static image/file, notebook code, HTML, BI/dashboard editing, slides/docs export, or no rendered artifact.
   ...
   - Use static Python charting only when the user explicitly asks for Python or when the selected delivery surface explicitly requires static image assets, such as portable HTML, PDF, Google Docs, Google Slides, or notebook-oriented output. Read `references/seaborn-templates.md` during the current task before every static Python implementation, revision, review, QA pass, or HTML report visual. Adapt the closest template and export the format expected by the selected delivery surface. Portable static HTML reports use PNG chart images.
...
- For data visualization requests where the user has not chosen a file, notebook, BI/dashboard, report, slide, docs, HTML, or no-render output surface, render with Data Analytics MCP chart widgets by default when the MCP widget tools are available; when the user explicitly chooses another surface, keep this skill's output to chart selection, data planning, implementation guidance, and QA for that surface.
