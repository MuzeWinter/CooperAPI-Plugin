...
WORKFLOW_SOURCE_REFERENCE = SKILL_ROOT.parents[1] / "references/workflow-source-resolution.md"
SPECIALIST_SKILLS = {
    "buyer-investor-list",
...
SHARED_SOURCE_SKILLS = {
    "buyer-investor-list": {
        "deal_materials",
        "relationship_counterparty_context",
        "market_data_public_sources",
        "models_workbooks_templates",
    },
    "capital-markets-issuance": {
        "deal_materials",
        "market_data_public_sources",
...
class InvestmentBankingStateHelpersTest(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_init_uses_namespaced_state_dir_and_creates_only_expected_files(self) -> None:
        ...
        self.assertIn(f"Investment Banking state directory: {self.state_dir}", proc.stdout)
        self.assertEqual(
    ...
    def test_init_overwrite_recreates_templates(self) -> None:
        ...
        self.assertIn(
            "# Banker Profile And Deliverable Style",
            (self.state_dir / "user-context.md").read_text(),
        )
        self.assertEqual(
            json.loads((self.state_dir / "onboarding-state.json").read_text()),
            {
                ...
                "hero_prompt_choice": {
                    "status": None,
                    "options": ["meeting-prep", "deal-process-tracker", "comps-valuation"],
                    "selected_skill": None,
    ...
    def test_preflight_reports_initialized_state_and_empty_categories(self) -> None:
        ...
        self.assertIn("Banker Profile And Deliverable Style", payload["empty_categories"])
    ...
    def test_preflight_excludes_populated_categories(self) -> None:
        ...
        user_context_path.write_text(
            user_context_path.read_text(encoding="utf-8").replace(
                ...
                "## Saved Links And Context\n\n- Coverage templates: https://example.com/templates",
                1,
            ),
            encoding="utf-8",
        )
        ...
        self.assertEqual(proc.returncode, 0, msg=proc.stderr)
        payload = json.loads(proc.stdout)
        self.assertNotIn("Banker Profile And Deliverable Style", payload["empty_categories"])
        self.assertIn("Mandate And Process Conventions", payload["empty_categories"])
        self.assertEqual(
            payload["saved_context"]["Banker Profile And Deliverable Style"],
            "- Coverage templates: https://example.com/templates",
        )
    ...
    def test_plugin_memory_reference_keeps_explicit_writes_narrow(self) -> None:
        ...
        self.assertIn("Do not save raw deal documents, live bidder status", memory_text)
    ...
    def test_source_selecting_specialists_use_shared_lazy_resolution_reference(self) -> None:
        ...
        for skill, categories in sorted(SHARED_SOURCE_SKILLS.items()):
            text = (SKILL_ROOT.parent / skill / "SKILL.md").read_text(encoding="utf-8")
            with self.subTest(skill=skill):
                self.assertIn("../../references/workflow-source-resolution.md", text)
    ...
    def test_preflight_stops_after_hero_workflow_selection(self) -> None:
        ...
        onboarding_state["hero_prompt_choice"]["selected_skill"] = "meeting-prep"
        onboarding_path.write_text(json.dumps(onboarding_state), encoding="utf-8")
    ...
    def test_preflight_exposes_four_step_progress_and_hero_options(self) -> None:
        ...
        self.assertEqual(
            ...
            ["meeting-prep", "deal-process-tracker", "comps-valuation"],
        )
    ...
    def test_automation_reference_keeps_setup_explicit_and_runtime_checked(self) -> None:
        ...
        self.assertIn("weekly-deal-process-review", config_text)
        self.assertIn(
        ...
        self.assertIn("Run a read-only weekly Investment Banking source check.", config_text)
        self.assertIn(
            "Report only: Upcoming Deadlines, Stale Sources, and Missing Inputs.", config_text
        )
        self.assertIn("Do not invent follow-ups.", config_text)
        self.assertIn("does not perform broad research or draft analysis;", automation_text)
        self.assertIn(
    ...
    def test_onboarding_reference_uses_four_step_defaults_and_hero_copy(self) -> None:
        ...
        intake_text = (SKILL_ROOT.parents[1] / "references/deliverable-intake-policy.md").read_text(
            encoding="utf-8"
        )
        ...
        self.assertIn("Reader-facing output: polished HTML report", onboarding_text)
        self.assertIn("Word document (.docx)", onboarding_text)
        self.assertIn("Audience: internal deal team", onboarding_text)
        self.assertIn("models, trackers, workbook updates, deck requests", onboarding_text)
        self.assertIn("1. **Prepare For A Coverage Meeting:**", onboarding_text)
        self.assertIn("2. **Reconstruct A Public M&A Process:**", onboarding_text)
        self.assertIn("3. **Build A Public Comps Valuation:**", onboarding_text)
        self.assertIn("Keep live deal details in the active workflow.", onboarding_text)
    ...
    def test_source_category_config_defines_static_catalog_only(self) -> None:
        ...
        self.assertEqual(config["schema_version"], "investment_banking_source_category_config.v1")
        self.assertEqual(
    ...
    def test_preflight_echoes_saved_source_route_without_claiming_readiness(self) -> None:
        ...
        onboarding_state["connector_confirmation"]["deal_materials"] = saved_route
        onboarding_path.write_text(json.dumps(onboarding_state), encoding="utf-8")
        ...
        self.assertEqual(proc.returncode, 0, msg=proc.stderr)
        plan = json.loads(proc.stdout)["source_category_plan"]
        category = plan["categories"]["deal_materials"]
    ...
    def test_preflight_keeps_missing_saved_source_route_as_setup_gap(self) -> None:
        ...
        onboarding_path = self.state_dir / "onboarding-state.json"
        onboarding_state = json.loads(onboarding_path.read_text(encoding="utf-8"))
        onboarding_state["connector_confirmation"]["deal_materials"] = {"status": "missing"}
        onboarding_path.write_text(json.dumps(onboarding_state), encoding="utf-8")

        proc = self.run_script(PREFLIGHT_SCRIPT, "--codex-home", str(self.codex_home))

        self.assertEqual(proc.returncode, 0, msg=proc.stderr)
        category = json.loads(proc.stdout)["source_category_plan"]["categories"]["deal_materials"]
        self.assertEqual(category["confirmation_status"], "saved_unverified")
        self.assertTrue(category["setup_required"])
